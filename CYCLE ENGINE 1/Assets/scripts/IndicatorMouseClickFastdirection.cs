...
public class IndicatorMouseClickFastdirection : MonoBehaviour
{
    [Header("Rail")]
    ...
    public float returnSpeed = 2f; // 🔄 vitesse de recentrage
    ...
    void Update()
    {
        ...
        {
            // 🔄 Retour progressif vers le centre quand relâché
            positionNormalized = Mathf.MoveTowards(
                positionNormalized,
                0.5f,
                Time.deltaTime * returnSpeed * Mathf.Abs(positionNormalized - 0.5f)
            );

            // Snap final si très proche
